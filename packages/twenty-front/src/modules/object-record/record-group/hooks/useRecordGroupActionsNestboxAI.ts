...
export const useRecordGroupActions = ({
  ...
  const openAIWorkflowSetupModal = useCallback(() => {
    const modalId = `ai-workflow-setup-${recordGroupDefinition.id}`;
    openModal(modalId);
  }, [openModal, recordGroupDefinition.id]);
  ...
  const recordGroupActions: RecordGroupAction[] = [
    ...
    },
    {
      ...
      condition:
        !isCurrentRecordGroupNotFound &&
        currentIndex < visibleRecordGroupIds.length - 1,
      callback: openAIWorkflowSetupModal,
    },
  ];
