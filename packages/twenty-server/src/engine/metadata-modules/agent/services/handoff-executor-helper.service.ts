...
export class HandoffExecutorHelperService {
  ...
  async generateMcpToolsForAgent(targetAgent: any): Promise<any> {
    this.logger.log(
      `🔧 GENERATING MCP TOOLS for handoff target: ${targetAgent.name}`,
    );
    const mcpTools =
      await this.mcpToolsHandlerService.generateMcpToolsForAgent(targetAgent);

    this.logger.log(
      `Successfully loaded MCP tools for agent ${targetAgent.id}`,
    );
...
Use your specialized tools to complete this request, then provide a clear, comprehensive response that summarizes what you accomplished. Always respond with text after using tools to explain the results.

Always end your interaction with a comprehensive text message explaining the results, even after using tools.`;
  ...
  async executeAiGeneration(aiRequestConfig: any): Promise<string> {
    this.logger.log(
      `🚀 EXECUTING HANDOFF: Running with ${Object.keys(aiRequestConfig.tools || {}).length} MCP tools`,
    );
    this.logger.log(
      `🛠️ TOOLS PASSED TO AI: ${Object.keys(aiRequestConfig.tools || {}).join(', ')}`,
    );
    ...
    this.logger.log(
      `📝 AI Response length: ${textResponse.text?.length || 0} characters`,
    );
    ...
    if (
      (!textResponse.text || textResponse.text.trim() === '') &&
      textResponse.toolResults &&
      textResponse.toolResults.length > 0
    ) {
      const toolSummary = textResponse.toolResults
        .map((tr: any, index: number) => {
          return `Tool ${index + 1}: ${JSON.stringify(tr, null, 2)}`;
        })
        .join('\n');
      const fallbackPrompt = `The tools response is 
      ${toolSummary}`;

      this.logger.log(`🔄 Alternative response: `, fallbackPrompt);
