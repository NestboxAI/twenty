...
export class ApiKeyCreateCommand extends CommandRunner {
  ...
  private async enableDashboardFeatureFlag(workspaceId: string): Promise<void> {
    try {
      await this.featureFlagService.enableFeatureFlags(
      ...
      this.logger.log(
        `✅ Dashboard V2 feature flag enabled for workspace ${workspaceId}`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to enable Dashboard V2 feature flag for workspace ${workspaceId}:`,
        error.message,
      );
      throw error;
    }
  ...
  async run(
    ...
    try {
      ...
      /**
       * Enable feature flags to support extended features
       */
      await this.enableDashboardFeatureFlag(workspace.id);
